...
[[developer/ievent/dns|dns]] it reports what became of each one. Two lines per
event is how you see ordering and what a position cost, and it is noise
...
the point of being here twice. The `age` field it prints is time since the
source decoded the event, not time since this position saw it - so the two lines
for one event give you a position's cost by subtracting one `age` from the
other, not by a stopwatch this plugin starts itself.
