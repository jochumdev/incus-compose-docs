...
## Why It Sits In Front Of The Enricher

It sits first, ahead of [[architecture/ievent/enricher|the enricher]], and that
is the whole point. An `instance-updated` storm - somebody editing devices, a
script walking a project - costs two reads rather than one per event. Collapsing
_behind_ the enricher would pay for every event in the burst and throw all but
one away.
...
Collapsing needs all three: `State` still `StateOk` (nothing has already
dropped or failed this event), the chain `ChainWarm` (the fleet has been read
whole at least once), and `Want.Debounce`. A cold start never collapses -
everything before the first full sweep goes straight through, in order, so
what that first sweep reads is everything Incus sent, not whatever survived a
window. An event already dropped or failed is not held either: it already
reports something, so delaying it buys nothing.

An event that lands for a key with an open window but does not itself qualify
to collapse closes that window first and hands on whatever it held, then goes
on itself - the older event arrived first, so it leaves first.
...
`Handle` does not block on a full inbox; it drops the event on the spot,
marked and handed straight on, same as a superseded one - so the observers
behind still see it, and see debounce as the reason it is gone.

## What A Command To Finish Does That A Cancelled Context Does Not

A `Command` asking this plugin to finish drains the inbox and closes every
open window, whatever its deadline, before answering: everything still held
gets handed on first, the source only hears back once it has. A cancelled
context is not that - it is an abort, and whatever is still held goes nowhere.
...
This plugin is in front of the enricher and still gets the whole table, which is
the point of it being the same one for everybody: what may be collapsed is
decided by plugins that all sit behind here. See
[[architecture/ievent/index|the contract]] for why the zero value vetoes.
...
[[architecture/ievent/log|log]] and the observers behind still see every event
