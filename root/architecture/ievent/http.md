...
`/ready` is the interesting one. [[architecture/ievent/dns|dns]] raises
`dns/ready` through `Command`, and http folds it like any other event - one
path, in order against the events that caused it, which is the same argument the
end of a round rests on. Edges rather than a level, so whoever folds them starts
not-ready: the truth before anything has been read.
...
`ActionDisconnected`: readiness does not imply the stream is up, and the
reverse doesn't hold either, which is why `/ready` checks both and fails on
...
`Handle` folds an event into those atomics and calls `next` immediately, on
the caller's goroutine - no inbox, like [[architecture/ievent/log|log]].
Staying synchronous costs three atomic stores, which is cheaper than owning a
goroutine and a channel to protect work this small.
...
The routes carry no method prefix: `go tool pprof` resolves symbols with a
POST, and the trailing slash on `/debug/pprof/` is what routes `/heap`,
`/goroutine` and `/allocs` to `Index`. The write timeout is switched off
whenever `Pprof` is on - `?seconds=30` only answers once the profile is
finished, and the usual timeout would cut everything longer than itself and
hand back a truncated profile.

## Finishing Is A Command Too

`in` and `out` are this plugin's own channel pair, separate from the chain of
events: the source asks http to finish through a `Command`, and http answers
on `out` - its own channel so the question arrives whatever else is going on.
...
With no `Listen`, `Run` still waits on that same `Command` instead of
returning outright - the source can't otherwise tell a plugin waiting quietly
from one that's wedged.
