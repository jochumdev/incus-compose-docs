...
what lets [[architecture/ievent/debounce|debounce]] sit in front and collapse a
burst before it costs a read.
...
Except a delete: Incus refuses to delete a profile still expanded into
anything, so the event names a profile no instance uses any more, and fanning
out would read a whole project's instances to learn nothing changed.
...
share a read: the key each flight is filed under is prefixed with which kind
it is, so the two can never collide.

## Every Event Carries Its Project's Labels Too

Before any of the four paths above, an event whose action somebody wants
`EnrichedProject` for gets the owning project's own configuration attached -
not a read of its own, but what the round already holds from its project
listing, read once per whole-fleet pass rather than again each time one of its
instances moves.
...
project has not been read" - both would otherwise arrive as the same empty
map. An event naming a project the model does not hold restarts the round,
once per project rather than once per event, because the fleet changed shape
since the round last came past and the next one is minutes away; the restart
re-emits that project's instances with their labels attached once it lands,
which the archive lets through because the labels are part of what makes an
event news.
...
`network-updated` that would have fixed it fanned out over
`instancesOn` - everyone already known to be there, which is everyone except it.
Nothing looks again: a wire arriving is not an instance event, and a lease
writes only `volatile.*`, which is stripped before the comparison. The record
answered on one wire and was missing on the other for the life of the process.
...
so a stop is never absorbed by the update in front of it, and whether a read
landed, because "no networks" and "nobody asked for networks" are different
answers.
...
from under it would let the delete overtake the update in front of it.

A fan-out event has no place to keep. They are minted in one burst, so their
...
goes in only if what came back was worth it. On completion the real events settle
first, so the invented one is compared against what they just filed - which is how
a live event and a fan-out on one key stay one event.
...
front holds up what is behind it, but never holds up their reads - those are
already running. Throughput is the pool's; latency is the slowest read still in
flight.

Every plugin behind here inherits that order for free rather than each rebuilding
it. A `create` landing after the `delete` that superseded it is a correctly
locked wrong answer, and nothing downstream can undo it.
...
during it. `debounce` in front and per-key coalescing behind make that a small
number. A timeout raised far enough would move that bound.

## The Key First, The Round Behind It
...
times. **Nothing retries inline** - the events queued behind that one would wait
out the whole delay.
...
as the key keeps failing. Stopping would mean waiting on a round that is
minutes away by design, and the common case here is an instance read that
landed between its start and its lease - a gap nothing else ever announces, so
nothing but another read closes it.

A wire is not re-read at all. The fan-out mints `instance-updated`, so handing it
a network would have the daemon asked for an instance named after the wire -
...
**Cold until the first pass warms it.** An instance read arriving before
`warm` is held rather than issued - joined into a flight the same way a
concurrent read on the same key would be, just never submitted - and released
in arrival order the moment the round's network phase lands. Only instance
reads wait: a network read patches the one wire it names directly, and a
profile or network fan-out only ever names instances the model already holds,
which is nothing while cold either way. Once warm, it never goes cold
again - a reconnect leaves the wires exactly as stale as everything else this
plugin holds, which is what the pass that follows one is for.
...
whatever went wrong. Every other round is paced, between projects and between the
reads inside one - and paced between rounds too, or a fleet read in no time is
read again immediately, for ever.
...
in the record, so it survives - and it had to, because nothing would put it back:
the next round reads it unchanged and emits nothing.

**A prune emits nothing.** The archive entry goes with the model entry, so a name
that comes back is news again. Nothing downstream is told, because nothing
downstream is being spoken for - see
[[architecture/ievent/dns|dns]], which reads Incus itself for the same reason.

**A listing that failed prunes nothing.** An empty answer and a daemon that would
not answer arrive the same way, and one of them means every name in the project.

`enricher/sweep-end` is raised through `Command`, which enters at the head of the
chain, because everything _in front_ of here has to see it too. It says a round
...
| `ProjectDelay` | 30s between one project of a round and the next, and between rounds |
| `ReadDelay`    | 5s between the reads inside one project                             |
| repair delay   | 1s, three attempts per key                                          |
...
- **A state read that fails is still nil**, but no longer silent: it records as a
  running instance holding no address, which is a key owed a re-read like any
  other.
