---
date: 2026-08-17T00:06:52.000Z
dateCreated: 2026-08-14T11:46:35.000Z
leafwiki_created_at: "2026-08-14T11:46:35.000000000Z"
leafwiki_updated_at: "2026-08-17T00:06:52.000000000Z"
---

# ievent

The base `coredns` and `operator` share: read the Incus event stream, fill each
...
[[architecture/ievent/index|Architecture]] has a page per position, extracted
from the code. This page is why they are shaped that way; those are what each
one does.
...
at the front holds up what is behind it, but never holds up their reads, which
are already running. Throughput is the pool's; latency is the slowest read still
in flight.

Every plugin behind the enricher inherits that order for free rather than each
...
drops travel: `log` and the observers behind still see every event Incus sent,
and who dropped it.

## The enricher

A plugin at a position, not something ahead of the chain - which is what lets
`debounce` sit in front and collapse a burst before it costs a read.
...
cannot be placed, so the read is refused whole rather than stored without it.
A subset stored once stays: the instance is then not on that wire, and the
network event that would repair it only ever reaches what is already known to be
there.
...
the engine, starts the listener, and puts the forwarder behind itself, so `main`
...
what it published becomes worth answering from, and `http` folds it like any
other event. Not a pointer to `dns`, not a registry, not an interface the chain
is walked for: one path, in order against the events that caused it, which is
the same argument the end of a round rests on. Edges rather than a level, so
whoever folds them starts not-ready - the truth before anything has been read.
...
`dns` is also what turns the chain warm, on `enricher/sweep-end`. It is the last
consumer, so warm set there says the whole chain has caught up with the round
rather than merely that a listing finished - and warm is a claim that there is
something worth acting on, which only the last position can make.
...
as a repeat. It excludes when the event was decoded and what the chain was doing,
neither of which is about the subject, and includes the action, so a stop is
never absorbed by the update in front of it.

An event saying nothing new goes nowhere, whoever sent it - and most of what a
busy fleet emits is one, since `volatile.*` is stripped before the comparison and
a lease renewal writes nothing else.
...
would let it overtake the event in front of it.

## The key first, the round behind it

A failed read fails its event immediately and reads that one key again a second
later, three times. Nothing retries inline: the events queued behind that one
...
round that way so everything _in front_ of it sees it too. The fan-out goes
forward through `next` instead, already enriched - sending it round would have
...
what arrived during it. `debounce` in front and per-key coalescing behind make
that a small number. A timeout raised far enough would move that bound.
...
- **The plugins share one context**, so a shutdown cancels all of them at once
  and one can drain before the plugin in front has handed its last event over. A
  chain-shaped shutdown, each stopping after the one that feeds it, is what the
  source and the plugins already do between them.
- **`TestShutdownHandsOnWhatItHolds` fails about one run in ten** with "the
  enricher never answered the drain". Not introduced by the last change to the
  enricher - 3 runs in 20 failed without it and 2 in 20 with it. The shape fits
  `raise` blocking on a full `CommandOut` once the harness has stopped reading
  it, which would keep `Run` from ever reaching the drain branch.
- **Upstream metrics label off `metrics.WithServer`**, which reads a context key
  ...
  friends carry `server=""` for good. Ours does not: `ecs_view` takes the name as
  a field at construction. There is no fix for theirs short of a Corefile.
- **Any upstream plugin links `core/dnsserver`**, because each carries a
  ...
  nearly the same set - and it costs 8.7 MiB of binary and nothing at runtime.
  A `-light` build is `main` with `queryChain` returning nil, which is about 200
